...
export function DashboardFilters({ currentQuarter }: { currentQuarter: string }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 mb-8">
      {/* Quarter switcher — horizontal scroll on mobile */}
      <div className="flex items-center bg-gray-100 rounded-xl p-1 overflow-x-auto shrink-0">
        {quarters.map((q) => (
          <button key={q} onClick={() => setQuarter(q)}
            className={`px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${
              currentQuarter === q ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"
            }`}>
            {q}
          </button>
        ))}
